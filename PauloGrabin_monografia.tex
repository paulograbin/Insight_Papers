...
\begin{itemize}
	...
	\item O aplicativo para a plataforma Android é dívido em quatro módulos:
		\begin{description}
			\item Módulo Bluetooth: Responsável por gerenciar a comunicação com a luva PERCEPT.
			\item Módulo Percept: Aplicação encarregada de responder a eventos como leitura de tag e pressionamento de botões. Quando ocorre a leitura de uma tag, esse módulo manda seu indentificador para o servidor, através de uma conexão sem fios, para que o cálculo da rota seja realizado.
	...
	\item O servidor armazena o layout dos ambientes e calcula as rotas até o destino escolhido pelo usuário. Através da ferramenta Quamtum GIS, cada andar é individualmente mapeado em um grafo onde cada nó representa um possivel destino e cada aresta um caminho disponível. Todos os dados são armazenados em uma base de dados Postgre. O servidor calcula a rota mais curta até o destino escolhido, formulando instruções de texto, quando recebe uma requisição de um cliente Android. Uma instrução de deslocamento 
\end{itemize}
